...
<div class="container">
    ...
    @endif
    <form method="post" action="/UpdateAssets">
        @csrf()
        ...
        <div class="row form-group m-auto col-5">
            ...
            <select name="assettype" class="form-control">
                ...
                @foreach($asp as $i)
                <option value="{{$i->id}}" {{($i->id == $data->assettype_id)?'selected':''}}>{{$i->name}}</option>
                @endforeach
            </select>
            @if($errors->has('assettype'))
            <label class="text-danger">{{$errors->first('assettype')}}</label>
            @endif
        </div>
        <div class="row form-group m-auto col-5">
